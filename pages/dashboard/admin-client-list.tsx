...
//         const data = await response.json();
//         console.log(data); // Check here if 'images' exists in the client object
...
//   return (
//     <DashboardLayout>
//         <div className="w-full truncate font-serif max-w-4xl p-6 bg-[var(--background-color)] text-[var(--text-color)] rounded-lg shadow-lg overflow-y-auto w-full md:w-3/4 lg:w-2/3 xl:w-1/2 mx-auto" >
//           <h1 className="text-2xl italic font-serif mb-10 text-center">Client List</h1>
//           <input
//             type="text"
//             value={searchTerm}
...
//           {filteredClients.length > 0 ? (
//             <div className="">
//               <table className=" py-2 px-4 truncate  text-xl font-bold max-w-xs bg-[var(--background-color)] text-[var(--text-color)] border-[var(--border-color)] ">
//                 <thead>
//                   <tr>
//                     <th className="py-2 px-4 text-left">Name</th>
//                     <th className="py-2 px-4 text-center">Actions</th>
...
//                       <td className="py-2 px-4 truncate text-lg font-semibold bg-transparent text-black-800 rounded-md flex items-center max-w-xs">
//                         <div className={`w-10 h-10 max-w-xs flex items-center justify-center ${colors[index % colors.length]} text-white rounded-full mr-3`}>
//                           {client.fullName ? client.fullName.charAt(0).toUpperCase() : client.email.charAt(0).toUpperCase()}
//                         </div>
//                         {client.fullName ? client.fullName : client.email}
//                       </td>
...
import Modal from '@/components/Modal';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faChevronDown, faChevronUp, faDumbbell, faWeightHanging, faHeartbeat, faDollarSign, faDrumstickBite, faHeartCircleBolt, faUsers, faEyeSlash, faEye, faRedoAlt, faChevronLeft, faChevronRight } from '@fortawesome/free-solid-svg-icons';
import { fa1, fa2, fa3, fa4, fa5, fa6 } from '@fortawesome/free-solid-svg-icons';
import { IconDefinition } from '@fortawesome/fontawesome-svg-core';
import LoadingSpinner from '@/components/LoadingSpinner';
const dayIcons: Record<string, IconDefinition> = {
...
const AdminClientList: React.FC = () => {
  ...
  const [selectedClientInfo, setSelectedClientInfo] = useState<string | null>(null);
  ...
  const handleInitialClick = (clientInfo: string) => {
  ...
  return (
    <DashboardLayout>
        <div className="w-full truncate font-serif  p-6 bg-[var(--background-color)] text-[var(--text-color)] rounded-lg shadow-lg overflow-y-auto w-full md:w-3/4 lg:w-2/3 xl:w-1/2 mx-auto" >
          <h1 className="text-2xl italic font-serif mb-10 text-center">Client List</h1>
          <input
            type="text"
            value={searchTerm}
                ...
                <tbody>
                  {currentClients.map((client, index) => (
                    <React.Fragment key={index}>
                      ...
                      <td className="py-2 px-4 max-w-xs truncate flex items-center text-lg font-semibold bg-transparent text-black-800 rounded-md">
                        {/* <div className={`w-10 h-10 max-w-xs flex items-center justify-center ${colors[index % colors.length]} text-white rounded-full mr-3`}>
                          {client.fullName ? client.fullName.charAt(0).toUpperCase() : client.email.charAt(0).toUpperCase()}
                        </div> */}
                        <button
                          className={`w-10 h-10 max-w-xs flex items-center justify-center ${colors[index % colors.length]} text-white rounded-full mr-3`}
                          onClick={() => handleInitialClick(client.fullName ? client.fullName : client.email)}
                        >
                          {client.fullName ? client.fullName.charAt(0).toUpperCase() : client.email.charAt(0).toUpperCase()}
                        </button>
                        <span className="truncate">
                          {client.fullName ? truncate(client.fullName, 8) : truncate(client.email, 8)}
                        </span>
                      </td>
                        <td className="py-2 px-4">
                          <div className="flex flex-row justify-center items-center space-x-3">
          ...
          {/* {renderPagination()} */}
            ...
            <button
              ...
              className={`py-2 px-4 ${currentPage === 1 && 'opacity-50 cursor-not-allowed'}`}
              disabled={currentPage === 1}
            >
              <FontAwesomeIcon icon={faChevronLeft} className="text-2xl text-blue-500" />
            </button>
            <span className="text-lg font-semibold">{`Page ${currentPage} of ${totalPages}`}</span>
            <button
              onClick={handleNextPage}
              className={`py-2 px-4 ${currentPage === totalPages && 'opacity-50 cursor-not-allowed'}`}
              disabled={currentPage === totalPages}
            >
              <FontAwesomeIcon icon={faChevronRight} className="text-2xl text-blue-500" />
            </button>
          </div>
        </div>
        ...
        <Modal onClose={handleModalClose}>
          <div className="p-6">
            ...
            <p className="text-lg">{selectedClientInfo}</p>
            <div className="flex justify-end mt-4">
              <button
