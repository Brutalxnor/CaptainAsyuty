...
//         setError('Failed to fetch data. No exercises assigned yet.'); // Friendly error message
//         setClientData({ fullName: userEmail, exercises: [], date: new Date().toISOString() }); // Allow app to proceed
//       } finally {
//         setLoading(false);
//       }
//     };
...
//       setMeals(mealsList);
//       setShowMeals(true); // Set to true to display the meals section
//     } catch (error: unknown) {  // explicitly setting error type to unknown
//         if (error instanceof Error) {
//           // Now TypeScript knows that error is an instance of Error, so you can access error.message safely
//           console.error('Error fetching meals:', error.message);
//           setError(error.message);
//         } else {
//           // In case it's not an Error object, log it or handle it accordingly
//           console.error('Unknown error fetching meals:', error);
  ...
  useEffect(() => {
    const fetchClientData = async () => {
      ...
      try {
        ...
        setClientData({ fullName: userEmail, exercises: [], date: new Date().toISOString() }); // Allow app to proceed even with error
      } finally {
        setLoading(false);
      }
    };
