...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    // Ensure paymentDate and registrationEndDate are valid dates
    const paymentDate = new Date(clientRecord.paymentDate);
    const registrationEndDate = new Date(clientRecord.registrationEndDate);
    const monthsRegistered = clientRecord.monthsRegistered;
    ...
    // Return the updated response, including the hasVisitedClientInfoPage field
    res.status(200).json({
      hasPaid: clientRecord.hasPaid,
      paymentDate: paymentDate.toISOString().split('T')[0], // Format as 'YYYY-MM-DD'
      registrationEndDate: registrationEndDate.toISOString().split('T')[0],
      monthsRegistered,
      daysLeft, // Add days left to the response
      hasVisitedClientInfoPage: clientRecord.hasVisitedClientInfoPage || false, // Add hasVisitedClientInfoPage (default to false if not present)
    });
  } catch (error: any) {
    console.error('Error checking payment status:', error.message);
    res.status(500).json({ message: 'Error checking payment status', error: error.message });
